...
async function processAttendanceItem(user, item, status) {
  let res = await logAttendance(user, item, status);
  if (res === null && item.toLowerCase().includes(' and ')) {
    const parts = item.split(/\s+and\s+/i);
    const subsPromises = parts.map(p => logAttendance(user, p, status));
    const subsResults = await Promise.all(subsPromises);
    const validSubs = subsResults.filter(Boolean);
...
async function handleAttended(user, rawText) {
  ...
  const results = await Promise.all(items.map(item => processAttendanceItem(user, item, 'present')));
  return results.join('\n\n');
}
...
async function handleMissed(user, rawText) {
  ...
  const items = rawText.split(',').map(s => s.trim()).filter(Boolean);
  const results = await Promise.all(items.map(item => processAttendanceItem(user, item, 'absent')));
  return results.join('\n\n');
}
...
async function handleDeemed(user, rawText) {
  ...
  const items = rawText.split(',').map(s => s.trim()).filter(Boolean);
  const results = await Promise.all(items.map(item => processAttendanceItem(user, item, 'deemed')));
  return results.join('\n\n');
}
