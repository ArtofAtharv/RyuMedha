...
CREATE TABLE subjects (
    ...
    -- Constraint: academic subjects must link to a course
...
CREATE TABLE attendance_logs (
    ...
    -- Prevent duplicate entries for same subject on same date
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE al.status = 'present') AS total_present,
    COUNT(*) FILTER (WHERE al.status = 'absent') AS total_absent,
    COUNT(*) FILTER (WHERE al.status = 'cancelled') AS total_cancelled,
    COUNT(*) FILTER (WHERE al.status IN ('present', 'absent')) AS total_counted_lectures,
    ROUND(
        (COUNT(*) FILTER (WHERE al.status = 'present')::NUMERIC / 
         NULLIF(COUNT(*) FILTER (WHERE al.status IN ('present', 'absent')), 0)) * 100, 
        2
    ) AS attendance_percentage
FROM attendance_logs al
JOIN subjects s ON al.subject_id = s.id
GROUP BY al.profile_id, al.subject_id, s.name, s.type;
