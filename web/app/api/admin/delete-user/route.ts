  ...
  try {
    ...
    try {
      ...
      if (exportData?.profiles && Array.isArray(exportData.profiles)) {
        const found = exportData.profiles.find(
          (p: { id: string; display_name?: string; email?: string; whatsapp_number?: string }) => p.id === profileId
        )
        if (found) {
          profileExists = true
    ...
    if (!deleteCompleted) {
      ...
      const { error: deleteErr } = await supabaseAdmin.from("profiles").delete().eq("id", profileId)
