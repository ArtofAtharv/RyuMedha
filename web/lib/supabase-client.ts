...
export function createAppClient(url: string, key: string, options?: Record<string, unknown>): AppSupabaseClient {
...
export function getAppClient(options?: Record<string, unknown>): AppSupabaseClient {
  return createAppClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, options)
}
