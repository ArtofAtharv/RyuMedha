...
export function InteractiveAttendanceGrid({
  initialData,
  subjectsInfo,
  token,
  profileId,
  ...
  const mergedData = useMemo(() => {
    ...
    return academicSubjects
      .map((sub) => {
        ...
        const expectedTotal =
          sub.expected_total_lectures || getSourceCourse(sub.source_course_id)?.expected_total_lectures || 0
  ...
  const buildUpdatedRow = useCallback(
    (item: AttendanceData, subjectId: string, isUndo: boolean, targetStatus: string) => {
      ...
      const newDeemed =
        targetStatus === "deemed" ? Math.max(0, (item.total_deemed ?? 0) + delta) : (item.total_deemed ?? 0)

      const subInfo = subjectsInfo.find((s) => s.id === subjectId)
      const expectedTotal =
        subInfo?.expected_total_lectures || getSourceCourse(subInfo?.source_course_id)?.expected_total_lectures || 0
      const {
        pct,
        bunksRemaining,
        neededToRecover,
        ...
        currentMisses,
        remainingLectures,
      } = calculateAdvice(newPresent, newAbsent, newDeemed, expectedTotal)
  ...
  const performOptimisticUpdate = useCallback(
    (subjectId: string, isUndo: boolean, targetStatus: string) => {
      setData((prev) => {
  ...
  const performDatabaseUndo = useCallback(
    async (subjectId: string, targetStatus: string) => {
  ...
  const performDatabaseLog = useCallback(
    async (subjectId: string, targetStatus: string) => {
  ...
  if (!mergedData || mergedData.length === 0) {
    return (
      <div className="text-muted-foreground bg-card rounded-xl border py-10 text-center">
        <p>No academic subjects found.</p>
        <p className="mt-1 text-xs">Add an academic subject to start tracking attendance.</p>
      </div>
    )
  }
    ...
    >
      {mergedData.map((item, idx) => {
        ...
        if (pct === 0 && (item.total_present > 0 || item.total_absent > 0 || item.total_deemed > 0)) {
          pct =
            ((item.total_present + (item.total_deemed || 0)) /
              (item.total_present + item.total_absent + (item.total_deemed || 0))) *
            100
        }

        const subRecord = subjectsInfo?.find((s) => s.id === item.subject_id)
        const instructorName =
          getSourceCourse(subRecord?.source_course_id)?.instructor_name || subRecord?.instructor_name || undefined
