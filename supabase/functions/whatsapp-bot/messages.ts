...
export const MESSAGES = {
  ...
  attendance: {
    ...
    trackWarning: (count: number, plural: boolean)=>`\n🚨 Warning: You need to attend the next *${count}* class${plural ? 'es' : ''} to get back on track!`,
    multiMarked: (name: string, oldStatus: string, newStatus: string, total: number)=>`I already marked you *${oldStatus}* but I will mark you *${newStatus}* again, which makes your total attendance for *${name}* subject as *${total}* lectures. 🗓️\n\n`,
    impossibleGoal: (pct: number | string)=>`\n⚠️ *Goal:* Attending all lectures daily without fail will make your attendance *${pct}%*. Don't skip anymore!`,
