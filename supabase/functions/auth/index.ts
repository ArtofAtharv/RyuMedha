  ...
  try {
    ...
    if (action === 'request') {
      ...
      if (!cleanPhone.startsWith('+')) {
        return json({
          success: false,
          error: 'phone_number must include country code'
        }, 400);
      }
      console.log(`Checking registration for: '${cleanPhone}' (received: '${phone_number}')`);
      // Check if user is registered
      const { data: profile } = await supabase.from('profiles').select('id').eq('whatsapp_number', cleanPhone).single();
      if (!profile) {
        console.log(`User not found: ${cleanPhone}`);
        return json({
          success: true,
          status: 'not_registered',
          searchedFor: cleanPhone
        });
      }
      // Rate limiting: max 3 sends per 10 minutes
      const TEN_MINUTES_AGO = new Date(Date.now() - 10 * 60 * 1000).toISOString();
      const { data: existing } = await supabase.from('otp_codes').select('attempts, last_sent_at').eq('whatsapp_number', cleanPhone).single();
      const inWindow = existing && existing.last_sent_at > TEN_MINUTES_AGO;
      if (inWindow && existing.attempts >= 3) {
        return json({
          success: false,
          error: 'Too many OTP requests. Wait 10 mins.'
        }, 429);
      }
      // Generate OTP
    ...
    if (action === 'verify') {
      ...
      if (!phone_number || !otp) return json({
        success: false,
        error: 'Missing phone/otp'
      }, 400);
      const cleanPhone = phone_number.replace(/\s/g, '');
      console.log(`Verifying OTP for: '${cleanPhone}' (received: '${phone_number}')`);
      const { data: record, error: fetchErr } = await supabase.from('otp_codes').select('code, expires_at, used, attempts').eq('whatsapp_number', cleanPhone).single();
      if (fetchErr || !record) return json({
        success: false,
        error: 'No OTP found'
      }, 400);
      if ((record.attempts ?? 0) >= MAX_ATTEMPTS) return json({
        success: false,
        error: 'Too many attempts'
      }, 429);
      if (record.used) return json({
        success: false,
        error: 'OTP already used'
      }, 400);
      if (new Date(record.expires_at) < new Date()) return json({
        success: false,
        error: 'OTP expired'
      }, 400);
      if (record.code !== String(otp)) {
        ...
        return json({
          success: false,
          error: `Incorrect OTP. ${MAX_ATTEMPTS - newAttempts} attempts left.`
        }, 400);
      }
      // Mark used
      await supabase.from('otp_codes').update({
      ...
      // Get profile
      const { data: profile } = await supabase.from('profiles').select('id, display_name, email').eq('whatsapp_number', cleanPhone).single();
