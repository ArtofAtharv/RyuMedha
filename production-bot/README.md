...
## Purpose
Contains the core Node.js application for the Ryu Medha WhatsApp production-bot. It acts as the primary webhook receiver for Meta's WhatsApp API.
...
## Architecture
An Express.js server listens for incoming POST requests from Meta. Messages are parsed and sent to an NLP worker (`nlp_worker.js`) which determines the intent. The production-bot then interacts with the Supabase database using a JWT-scoped client to ensure Row Level Security (RLS). Responses are queued and sent back.

## Important Files
- `server.js`: The main Express server and webhook handler.
- `nlp_worker.js`: Handles intent recognition using NLP.js/Compromise.
- `smartRyuma.js`: Contains logic for specific production-bot commands and responses.
...
- Functionality is duplicated in `production-production-bot/` and `testing-production-bot/`.
